...
describe('Authentication controller integration', () => {
  ...
  */
  test('authenticates the same user and issues a fresh JWT', async () => {
    ...
    // Optional: verify that /auth/me works with this new token
    const response2 = await fetch(`${baseUrl}/auth/me`, {
      headers: { Authorization: `Bearer ${issuedToken}` }
    });
    const mePayload = await response2.json();
    expect(response2.status).toBe(200);
    expect(mePayload.user.email).toBe(credentials.email.toLowerCase());
  });
